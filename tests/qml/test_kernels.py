...
class TestKernelAlignment:
    ...
    def test_alignment_symmetric_labels(self):
        K = np.array([[1, 0.5, 0], [0.5, 1, 0], [0, 0, 1]])
        y = np.array([0, 0, 1])
    ...
    def test_evaluate_kernel_quality(self):
        K = np.array([[1, 0.5, 0.1], [0.5, 1, 0.2], [0.1, 0.2, 1]])
        y = np.array([0, 0, 1])
...
class TestTrainQSVM:
    ...
    def test_train_qsvm_basic(self):
        ...
        result = train_qsvm(X_train, y_train, n_qubits=2, feature_map="angle")
    ...
    def test_train_qsvm_with_test(self):
        ...
        result = train_qsvm(X_train, y_train, X_test, y_test, n_qubits=2)
