...
class TestCirqAdvanced:
    ...
    def test_quantum_teleportation(self):
        ...
        cirq_circuit.append([cirq.H(qubits[1]), cirq.CNOT(qubits[1], qubits[2])])

        # Alice's operations
        cirq_circuit.append([cirq.CNOT(qubits[0], qubits[1]), cirq.H(qubits[0])])

        # Bob's corrections (conditional)
        cirq_circuit.append(
            [cirq.CNOT(qubits[1], qubits[2]), cirq.CZ(qubits[0], qubits[2])]
        )
    ...
    def test_state_vector_accuracy(self):
        ...
        np.testing.assert_array_almost_equal(
            np.abs(original_state), np.abs(converted_state), decimal=10
        )
    ...
    def test_swap_test_circuit(self):
        ...
        )  # Regular SWAP instead of fractional
        cirq_circuit.append(cirq.H(qubits[0]))
