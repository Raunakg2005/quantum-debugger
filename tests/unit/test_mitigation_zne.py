...
def test_circuit_folding():
    ...
    assert (
        len(folded_3x.gates) == expected_gates
...
def test_zne_basic():
    ...
    result = zero_noise_extrapolation(
        circuit, scale_factors=[1.0, 2.0, 3.0], extrapolation_method="linear", shots=500
    )
    ...
    assert (
        result["mitigated_value"] >= result["unmitigated_value"] * 0.95
