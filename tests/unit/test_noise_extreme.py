...
def test_eigenvalue_bounds():
    ...
    noises = [DepolarizingNoise(0.5), AmplitudeDamping(0.7), PhaseDamping(0.6)]
    ...
    for noise in noises:
        ...
        assert np.all(
            eigenvalues >= -1e-10
        ), f"{noise}: Negative eigenvalue {min(eigenvalues)}"
        assert np.all(
            eigenvalues <= 1.0 + 1e-10
        ), f"{noise}: Eigenvalue > 1: {max(eigenvalues)}"
        assert (
            np.abs(np.sum(eigenvalues) - 1.0) < 1e-10
        ), f"{noise}: Eigenvalues don't sum to 1"
...
def test_hermiticity_after_many_operations():
    ...
    noises = [DepolarizingNoise(0.05), AmplitudeDamping(0.05), PhaseDamping(0.05)]
    ...
    for i in range(30):
        ...
        ), f"Step {i}: Lost Hermiticity, error={hermitian_error}"
...
def test_purity_bounds():
    ...
    for i, psi in enumerate(test_states):
        ...
        assert (
            0 <= final_purity <= 1.0 + 1e-10
        ), f"State {i}: Purity out of bounds: {final_purity}"
...
def test_decoherence_free_subspace():
    ...
    assert np.allclose(
        state_0.density_matrix, rho_0_initial, atol=1e-14
