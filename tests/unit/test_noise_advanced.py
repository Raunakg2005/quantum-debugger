...
def test_density_matrix_properties():
    ...
    noises = [DepolarizingNoise(0.2), AmplitudeDamping(0.3), PhaseDamping(0.15)]
...
def test_zero_noise_is_identity():
    ...
    # Apply zero noise
    noises = [DepolarizingNoise(0.0), AmplitudeDamping(0.0), PhaseDamping(0.0)]
...
def test_multi_qubit_noise():
    ...
    assert np.allclose(
        state.density_matrix, state.density_matrix.conj().T
    ), "Should be Hermitian"
...
def test_noise_composition():
    ...
    assert not np.allclose(
        rho_initial, rho_after_t1, atol=1e-10
    ), "T1 should modify state"
    assert not np.allclose(
        rho_after_t1, rho_final, atol=1e-10
    ), "T2 should further modify state"
...
def test_density_matrix_properties():
    ...
    for noise in noises:
        ...
        ), f"{noise} trace = {trace}, should be 1"

        # 3. Positive semi-definite (eigenvalues >= 0)
        eigenvalues = np.linalg.eigvalsh(rho)
        assert np.all(
            eigenvalues >= -1e-10
        ), f"{noise} has negative eigenvalues: {eigenvalues}"
...
def test_numerical_stability():
    ...
    assert np.isclose(
        trace, 1.0, atol=1e-8
    ), f"Trace drifted to {trace} after 100 steps"
...
def test_amplitude_damping_decay_rate():
    ...
    for gamma in gammas:
        ...
        assert np.isclose(
            p1, expected_p1, atol=1e-10
        ), f"γ={gamma}: P(1) = {p1}, expected {expected_p1}"
        assert np.isclose(
            p0, expected_p0, atol=1e-10
        ), f"γ={gamma}: P(0) = {p0}, expected {expected_p0}"
...
def test_phase_damping_coherence_decay():
    ...
    for gamma in gammas:
        ...
        assert np.isclose(
            final_coherence, expected_coherence, atol=1e-10
        ...
        print(
            f"✓ γ={gamma}: coherence={final_coherence:.4f} (correct), populations preserved"
        )
...
def test_zero_noise_is_identity():
    ...
    for noise in noises:
        ...
        noise.apply(state)
        assert np.allclose(
            state.density_matrix, rho_initial, atol=1e-15
...
def test_maximum_noise_limits():
    ...
    coherence = abs(state.density_matrix[0, 1])
    assert np.isclose(
        coherence, 0.0, atol=1e-10
    ), f"γ=1 should destroy coherence, got {coherence}"
