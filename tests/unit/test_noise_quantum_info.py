...
def test_error_syndromes():
    ...
    for name, noise in noise_types.items():
        ...
        syndrome = (rho[0, 0].real, rho[1, 1].real, rho[0, 1].real, rho[0, 1].imag)

        syndromes[name] = syndrome
        print(
            f"  {name:20s}: P0={syndrome[0]:.3f}, P1={syndrome[1]:.3f}, "
    ...
    depol_coherence = abs(syndromes["Bit flip (X)"][2]) + abs(
        syndromes["Bit flip (X)"][3]
    )
    phase_coherence = abs(syndromes["Phase flip (Z)"][2]) + abs(
        syndromes["Phase flip (Z)"][3]
    )
    amp_coherence = abs(syndromes["Amplitude"][2]) + abs(syndromes["Amplitude"][3])

    # At least one pair should be significantly different
    assert not np.isclose(
        depol_coherence, phase_coherence, atol=0.05
    ) or not np.isclose(
        depol_coherence, amp_coherence, atol=0.05
...
def test_channel_distinguishability():
    ...
    for i in range(len(purities)):
        for j in range(i + 1, len(purities)):
            assert not np.isclose(
                purities[i], purities[j], atol=0.01
...
def test_fidelity_under_concatenation():
    ...
    print(
        f"  Fidelities: {fidelities[0]:.4f} → {fidelities[10]:.4f} → {fidelities[-1]:.4f}"
    )
...
def test_stabilizer_preservation():
    ...
    for name, psi in states.items():
        ...
        print(
            f"  {name}: max diagonal={np.max(diag):.4f}, max off-diag={off_diag_max:.4f}"
        )
