...
def finite_difference_gradient(
    cost_function: Callable, params: np.ndarray, param_index: int, epsilon: float = 1e-7
) -> float:
    """
...
def compute_gradients(
    ...
    for i in range(len(params)):
        if method == "parameter_shift":
            ...
            gradients[i] = finite_difference_gradient(cost_function, params, i)
        else:
...
def gradient_descent_step(
    params: np.ndarray, gradients: np.ndarray, learning_rate: float
) -> np.ndarray:
    """
