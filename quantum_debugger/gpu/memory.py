    ...
    def get_memory_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "utilization": (
                    (used_bytes / device_mem[1]) * 100 if device_mem[1] > 0 else 0
                ),
            }
        except ImportError:
    ...
    def get_recommendations(self) -> Dict[str, str]:
        ...
        if utilization > 90:
            recommendations["critical"] = (
                "GPU memory >90% full. Enable gradient checkpointing or reduce batch size."
            )
        elif utilization > 75:
            recommendations["warning"] = (
                "GPU memory >75% full. Consider enabling mixed precision or gradient checkpointing."
            )
        elif utilization < 30:
            recommendations["info"] = (
                "GPU memory underutilized. Can increase batch size or model complexity."
            )

        if not self.gradient_checkpointing_enabled and utilization > 50:
            recommendations["suggestion"] = (
                "Gradient checkpointing disabled. Enable for ~50% memory reduction."
            )
...
def profile_memory(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        logger.info(
            f"Memory increase: {stats_after['used_mb'] - stats_before['used_mb']:.1f} MB"
        )
