...
class QuantumState:
    ...
    def __init__(
        self, num_qubits: int, state_vector: Optional[np.ndarray] = None, backend="auto"
    ):
        """
        ...
        if state_vector is not None:
            if len(state_vector) != self.dim:
                raise ValueError(
                    f"State vector size {len(state_vector)} doesn't match {self.dim}"
                )
    ...
    def _build_full_gate_matrix(
        ...
        if num_gate_qubits == self.num_qubits and target_qubits == list(
        ...
        for in_idx in range(self.dim):
            for out_idx in range(self.dim):
                ...
                if non_target_match:
                    ...
                    full_matrix[out_idx, in_idx] = gate_matrix[
                        gate_out_idx, gate_in_idx
                    ]
    ...
    def _partial_trace(self, keep_qubit: int) -> np.ndarray:
        ...
        for i in range(self.dim):
            for j in range(self.dim):
                ...
                if i_other == j_other:
                    rho[i_bit, j_bit] += (
                        self.state_vector[i] * self.state_vector[j].conj()
                    )
