...
class QuantumCircuit:
    ...
    def __init__(
        ...
        if (
            self.apply_noise
            and num_qubits > 10
            and noise_simulation_method == "density_matrix"
        ):
    ...
    def phase(self, theta: float, qubit: int):
        ...
        return self._add_gate(
            "PHASE", GateLibrary.PHASE(theta), qubit, {"theta": theta}
        )
    ...
    def cp(self, theta: float, control: int, target: int):
        ...
        return self._add_gate(
            "CP", GateLibrary.CP(theta), [control, target], {"theta": theta}
        )
    ...
    def toffoli(self, control1: int, control2: int, target: int):
        ...
        return self._add_gate(
            "TOFFOLI", GateLibrary.TOFFOLI, [control1, control2, target]
        )
    ...
    def _run_with_noise(
        self, shots: int, initial_state: Optional[QuantumState]
    ) -> dict:
        """
        ...
        for _ in range(shots):
            ...
            if initial_state:
                ...
                if (
                    hasattr(initial_state, "density_matrix")
                    and initial_state.density_matrix is not None
                ):
                    state = initial_state.copy()
                else:
                    state = NoisyQuantumState(
                        self.num_qubits,
            ...
            reference_state = NoisyQuantumState(
                self.num_qubits, use_density_matrix=True
            )
            for gate in self.gates:
            ...
            if hasattr(state, "density_matrix") and state.density_matrix is not None:
                ...
                fidelity = np.trace(
                    reference_state.density_matrix @ state.density_matrix
                ).real
