...
class Transpiler:
    ...
    def transpile(
        self, circuit_gates: List[Tuple], initial_layout: Optional[List[int]] = None
    ) -> List[Tuple]:
        """
        ...
        logger.info(
            f"Transpilation added {overhead} gates "
    ...
    def _apply_layout(
        self, gates: List[Tuple], layout: Optional[List[int]]
    ) -> List[Tuple]:
    ...
    def _find_swap_path(self, q1: int, q2: int, mapping: List[int]) -> List[Tuple]:
...
def transpile_circuit(
    circuit_gates: List[Tuple], backend_topology: Dict
) -> List[Tuple]:
    """
